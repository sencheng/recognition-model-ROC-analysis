...
#            label='p=%s'%params.pat_sep[data['pat_sep'][ind]]+',\u03C9=%s'%params.noise[data['noise'][ind]]
        ...
        ax.plot(fa_model,hit_model,label=label,color=data['color'][ind],linestyle=data['line'][ind])
        ax.plot(fa_obs,hit_obs,'o',color=data['color'][ind])
        ax.plot([0,1],[0,1],'k--')
        if barplot or change:
            ax1=axes[1]
...
#    axis_default(ax,'False alarm rate','Hit rate', limit=[[-0.01,1.01],[0,1.01]],legend=True,leg_size=12,aspect=True,labelsize=16,ticksize=14)
    if barplot:
        axis_default(ax1,'','Estimate',aspect=True,labelsize=16,ticksize=14)
#        ax1.set_xticklabels(["R","F"],fontsize=14)
#        xlim=ax1.get_xlim()
#        ax1.set_xticks([xlim[0]+width*2,xlim[-1]-width*2.5])
        x_values=np.arange(1,len(data['IDs'])+1).tolist()
        print(x_values)
        ax1.set_xticks(Rticks+Fticks)
        ax1.set_xticklabels(x_values*2)
        ax1.tick_params(axis='x', which='major', labelsize=14)  
    if change: 
        ax1.plot(X,F_all,'o-',color='green',label='F')
        ax1.plot(X,R_all,'o-',color='blue',label='R')
        axis_default(ax1,'Pattern separation','Parameter',aspect=True,labelsize=16,ticksize=14,legend=True,leg_size=12)
    fig.tight_layout()
    if save:
        plt.savefig(params.path+'/Figures/'+filename+".pdf", format='pdf', dpi=300)
compare_rocs(info_update,path1,figsize=(8,8),barplot=True,labeling='pat_sep',save=True)
#compare_rocs(info_update,path1,figsize=(10,10),barplot=True,labeling='cond',save=True)
